...
void fx_app_thread_entry(ULONG thread_input)
{
  ...
  ULONG bytes_read = 0;
  FX_FILE fx_file;
  sd_status = fx_file_open(&sdio_disk, &fx_file, "Test.txt", FX_OPEN_FOR_READ);
  ...
  sd_status = fx_directory_first_full_entry_find(&sdio_disk, name, &attributes, &size, &year, &month, &day, &hour,
    &minute, &second);

  sd_status = fx_directory_next_full_entry_find(&sdio_disk, name, &attributes, &size, &year, &month, &day, &hour,
    &minute, &second);
