...
void PWMDriver::Config(size_t sample_rate)
{
  ...
  Play(); // Here for testing without UART
}
...
void PWMDriver::Play()
{
  ...
  HAL_TIM_PWM_Start(m_timer_aux_pwm, m_right_channel);
...
void PWMDriver::Stop()
{
  ...
  HAL_TIM_PWM_Stop(m_timer_aux_pwm, m_right_channel);
...
void PWMDriver::TimerAuxPwmPeriodElapsedCallback()
{
  ...
  __HAL_TIM_SET_COMPARE(m_timer_aux_pwm, m_right_channel, m_sample_right);
}
